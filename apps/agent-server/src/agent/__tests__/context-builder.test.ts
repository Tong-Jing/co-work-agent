...
describe("ContextBuilder", () => {
  it("builds a system message combining instructions, memories, and skills, then appends history", async () => {
    ...
    const builder = new ContextBuilder(memory as any, skills as any, new SkillSelector());
    ...
    expect(gathered).toEqual({ memoryCount: 1, skillCount: 1, historyCount: 2 });
  ...
  it("omits memory and skills sections entirely when there is nothing relevant", async () => {
    ...
    expect(gathered).toEqual({ memoryCount: 0, skillCount: 0, historyCount: 0 });
